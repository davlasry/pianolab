...
export function SharedPlayhead({
    ...
    const scrollIfNeeded = () => {
        ...
        const thresholdPosition =
            scrollLeft + containerWidth * SCROLL_THRESHOLD;

        if (playheadX > thresholdPosition) {
            // Scroll to keep playhead at LEFT_MARGIN_RATIO from the left edge
            const newScrollLeft =
                playheadX - containerWidth * LEFT_MARGIN_RATIO;
            const clampedScrollLeft = Math.max(
                0,
                Math.min(newScrollLeft, scrollWidth - containerWidth),
            );
    ...
    useEffect(() => {
        if (barRef.current && containerRef.current) {
            ...
            // No offset needed - position playhead directly at the time position
            const centeredX = x;
    ...
    return (
        ...
        >
            ...
            {/*</svg>*/}
            {/* Vertical line */}
            <div className="-mt-px w-0.5 flex-1 bg-white" />
        </div>
    );
}
